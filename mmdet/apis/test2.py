...
def _save_adv_image(tensor_img, img_transform, filename, save_dir, img_meta=None, save_ext=".png"):
    # 1) 反归一化到 0~255
    denorm = img_transform[1](tensor_img.detach())  # (1,3,H,W) in 0~255

    # 2) 去掉 padding（用 img_shape；如果不给就不裁）
    if img_meta is not None and 'img_shape' in img_meta:
        h, w = img_meta['img_shape'][:2]
        denorm = denorm[:, :, :h, :w]

    # 3) 如需还原到原图大小（可选）
    if img_meta is not None and 'ori_shape' in img_meta:
        ori_h, ori_w = img_meta['ori_shape'][:2]
        np_tmp = denorm.squeeze(0).permute(1,2,0).clamp(0,255).cpu().numpy().astype('uint8')
        np_tmp = cv2.resize(np_tmp, (ori_w, ori_h), interpolation=cv2.INTER_LINEAR)
    else:
        np_tmp = denorm.squeeze(0).permute(1,2,0).clamp(0,255).cpu().numpy().astype('uint8')

    # 4) RGB->BGR & 写盘（保留原子目录结构）
    np_bgr = np_tmp[:, :, ::-1]
    base, _ = os.path.splitext(filename)
    out_path = os.path.join(save_dir, base + save_ext)
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    cv2.imwrite(out_path, np_bgr)
    return out_path
...
def multi_gpu_test(model, data_loader, tmpdir=None, gpu_collect=False, test_adv_cfg=None, adv_load_dir=None):
    ...
    for i, data in enumerate(data_loader):
        if adv_flag and (adv_load_dir is None):
            ...
            if save_adv and (save_dir is not None):
                # 注意：此处需要用“原图文件名”
                ori_name = sample['img_metas'][0]["ori_filename"]
                _save_adv_image(img_adv, img_transform, ori_name, save_dir, img_meta=sample['img_metas'][0], save_ext=save_ext)
